...
const REGELS = [
  ...
  // Wachten/aandacht is amber (index.css: semantisch oranje, met de
  // dark-mode-schaal erbij); Tailwinds orange-* heeft geen omgekeerde schaal
  // en zou naast amber een tweede, net andere waarschuwingskleur geven (P1).
  { naam: 'orange-NNN (gebruik amber-*: dé waarschuwingskleur, flipt in dark)', re: /(?<![\w-])(?:[\w-]+:)*(?:bg|text|border|ring|divide|from|via|to|fill|stroke|outline|decoration|accent|caret|shadow)-orange-\d+(?:\/\d+)?\b/g },
