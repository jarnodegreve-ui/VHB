 ...
 */
export function RecordOnbekend({ soort, onSluit }: { soort: string; onSluit: () => void }) {
  return (
    <Callout
      tone="info"
      role="status"
      title={`Deze ${soort} is niet beschikbaar`}
      action={<Button variant="secondary" size="sm" onClick={onSluit}>Sluiten</Button>}
    >
      <p className="text-body-sm">Ze bestaat niet meer, of je hebt er geen toegang toe.</p>
    </Callout>
  );
}
