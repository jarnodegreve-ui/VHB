...
export function LeaveBalanceCard({ balance, year, compact = false }: { balance: LeaveBalance; year: number; compact?: boolean }) {
  ...
  return (
    <div className={cn('rounded-[24px] border border-slate-100 bg-white/55 p-5 space-y-4', compact && 'p-4 space-y-3')}>
      <div className="flex items-baseline justify-between gap-3">
        <h5 className={cn('font-black text-slate-700 tracking-tight', compact ? 'text-sm' : 'text-base')}>Verlofbalans</h5>
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{year}</span>
      </div>

      <div className="space-y-2">
        <div className="flex items-baseline justify-between gap-3">
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Betaald verlof</span>
          <span className={cn('text-xs font-bold', overBudget ? 'text-red-500' : 'text-slate-600')}>
            <span className={cn('font-black', compact ? 'text-base' : 'text-lg')}>{balance.betaaldGebruikt}</span>
      ...
      <div className="flex items-baseline justify-between gap-3 pt-2 border-t border-slate-100">
        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Klein verlet</span>
        <span className="text-xs font-bold text-slate-600">
          <span className={cn('font-black', compact ? 'text-base' : 'text-lg')}>{balance.kleinVerletDagen}</span>
