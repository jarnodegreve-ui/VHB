 ...
 * het printblad.
 *
 *  - geen-bron  "Er zijn nog geen verlofgegevens geregistreerd."
...
export function bereikUitleg(def: RapportDefinitie, periode: Periode | null, bereik: RapportBereik): BereikUitleg {
  // Een rapport zonder periode (geen periode- of jaarfilter) heeft niets om tegen af te zetten.
  if (!periode) return { toestand: 'binnen', tekst: null };
  const toestand = bereikToestand(periode, bereik);
  if (toestand === 'binnen') return { toestand, tekst: null };
  if (toestand === 'geen-bron' || !bereik) return { toestand: 'geen-bron', tekst: `Er zijn nog geen ${def.bronNaam} geregistreerd.` };
