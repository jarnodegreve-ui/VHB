...
## Huisstijl (bindend)
- **Definitief logo** (pakket 2026-08): `brand/vhb-final-logo-package/` + `VHB-gebruiksrichtlijnen.md` — onderbroken ovale lus, VHB-monogram met gouden H-verbinding, naamregel als contouren. In de app uitsluitend via `<BrandLogo tone variant>` (inline SVG, kleuren bewust hard). Sizen op **breedte**: login `w-40 sm:w-64`, pre-app `w-40 sm:w-56`; sidebar `w-32 lg:w-36` (128/144 px — bewust onder de 180 px-richtlijn, keuze Jarno 30-08: groter was te groot, zonder naamregel wilde hij niet; niet kleiner). Laad-momenten gebruiken de lus-spinner (`BrandSpinner`), geen generieke draaicirkels. Naamregel: sidebar `naamregelSchaal={1.2} naamregelAfstand={26}`, login/pre-app `naamregelAfstand={26}` (Jarno 30-08 — bewuste afwijking van het master). Negatief (`tone="donker"`) is gedempt wit `#DCDFE2`, geen `#FFFFFF` (te fel op het zwart). Niet vervormen, lus-onderbreking niet sluiten, geen schaduw/gloed/verloop.
- Iconen/favicon in `public/` worden gegenereerd: `node scripts/brand-icons.mjs` (app-icoon = beeldmerk-negatief op VHB Black-tegel; tab-favicon = carbon monogram op goud-tegel, want de lus is op 16–32 px onleesbaar) — niet met de hand bewerken.
