...
## Huisstijl (bindend)
- **Definitief logo** (pakket 2026-08): `brand/vhb-final-logo-package/` + `VHB-gebruiksrichtlijnen.md` — onderbroken ovale lus, VHB-monogram met gouden H-verbinding, naamregel als contouren. In de app uitsluitend via `<BrandLogo tone variant>` (inline SVG, kleuren bewust hard). Sizen op **breedte**: volledig logo ≥ 180 px (login/pre-app: `w-56`/`w-64`), los beeldmerk ≥ 48 px. Sidebar = `variant="beeldmerk"` op `w-36` — het volledige logo was daar te groot (Jarno 30-08). Niet vervormen, lus-onderbreking niet sluiten, geen schaduw/gloed/verloop.
