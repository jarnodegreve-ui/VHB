...
- **Routing**: de URL is de bron (src/app/router.ts); elk scherm staat één keer in `src/app/routes.tsx` (pad, label, icoon, sectie, rollen) — sidebar, bottom-nav, ⌘K-palette, rol-guard en topbar-titel volgen daaruit. Nieuw scherm = één regel in ROUTES + `View`-type + een case in App.tsx. Overlays gebruiken `useHistoryDismiss` (terugknop sluit ze). Datalaag in `src/app/useAppData.ts`; App.tsx = auth/sessie/thema/toasts/schil.
