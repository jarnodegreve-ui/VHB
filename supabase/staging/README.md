...
70. `supabase/2026-09-21_updates_bijlagen.sql`, PDF-bijlagen bij een update: kolommen `bijlagen` (jsonb, check op hoogstens twee) en `bijlagen_tonen` (boolean, not null default false) op `public.updates`, plus de privé bucket `update-bijlagen`; raakt geen RLS, policies of grants, dus geen snapshot-update. Draai óók de bucket-statements, anders mislukt het uploaden op staging (op staging gedraaid op 21-09, post-condities groen: twee kolommen, check-constraint, bucket privé).
71. `supabase/2026-09-22_leave_beslisreden.sql`, reden bij het weigeren van verlof: nullable kolom `beslisreden` (text, check op hoogstens 500 tekens) op `public.leave`; raakt geen RLS, policies of grants, dus geen snapshot-update.
72. `supabase/2026-09-25_diversions_bijlagen.sql`, tot vijf PDF's bij een omleiding: kolom `bijlagen` (jsonb, check op hoogstens vijf) op `public.diversions`; de bucket `diversions` bestaat al en blijft privé (het statement zet hem desnoods opnieuw dicht). Bestaande PDF's (`<id>.pdf`, marker `"pdfUrl"`) blijven werken als slot 1 tot iemand die omleiding aanraakt. Raakt geen RLS, policies of grants, dus geen snapshot-update.
73. `supabase/2026-09-25_mail_log.sql`, verzendlog van de portaalmails: tabel `mail_log` (soort, moment, aantal, gelukt, fout, door; bewust zonder inhoud of adressen), RLS aan zonder policies, grants voor anon/authenticated ingetrokken. Raakt RLS en grants: daarna `beleid-drift.mjs --update` en de snapshot committen (op staging gedraaid op 25-09 via de Supabase-koppeling, post-condities groen: 7 kolommen, RLS aan, 0 grants voor anon/authenticated, check-constraint aanwezig).
...
- `UPSTASH_*`, `RATE_LIMIT_*`, `RETENTION_*` — defaults.
